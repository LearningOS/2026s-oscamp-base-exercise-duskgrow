...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in [2, 1] {
            ...
            if self.nodes.get_mut(&ppn).unwrap().entries[idx] & PTE_V == 0 {
                let new_ppn = self.alloc_node();
                self.nodes.get_mut(&ppn).unwrap().entries[idx] = (new_ppn << PPN_SHIFT) | PTE_V;
                ppn = new_ppn;
            } else {
                ppn = self.nodes.get_mut(&ppn).unwrap().entries[idx] >> PPN_SHIFT;
            }
        }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in [2, 1, 0] {
            let idx = Self::extract_vpn(va, level);
            let pte = self.nodes.get(&ppn).unwrap().entries[idx];
            if pte & PTE_V == 0 {
                return TranslateResult::PageFault;
            }
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                return TranslateResult::Ok(
                    ((pte >> PPN_SHIFT) << 12) | (va & ((1 << (12 + level * 9)) - 1)),
                );
            }
            ppn = pte >> PPN_SHIFT;
        }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        if self.nodes.get_mut(&ppn).unwrap().entries[idx] & PTE_V == 0 {
            let new_ppn = self.alloc_node();
            self.nodes.get_mut(&ppn).unwrap().entries[idx] = (new_ppn << 10) | PTE_V;
            ppn = new_ppn;
        } else {
            ppn = self.nodes.get_mut(&ppn).unwrap().entries[idx] >> 10;
        }
        let idx = Self::extract_vpn(va, 1);
        self.nodes.get_mut(&ppn).unwrap().entries[idx] = (pa >> 12) << 10 | flags;
    }
}
